...
func (woo *WcAdd) FindCat3_WithParam(Parent int, Name string, Slug string) (*Category3Base, error) {
	// fmt.Println(" woo.Cat3", woo.Cat3)
	for _, CatBase := range woo.Cat3 {
		...
		//fmt.Println("CatBase", CatBase.Cat3)
		FindCat, ErrorFind := findCategory3Base_WithParam(CatBase, Parent, Name, Slug)
		//fmt.Println("CatBase!!!", FindCat, ErrorFind)
		if ErrorFind == nil {
			return FindCat, nil
		}
...
// Поиск именно в ячейке
func findCategory3Base_WithParam(cat *Category3Base, Parent int, Name string, Slug string) (*Category3Base, error) {
	// fmt.Println("cat.Cat3", cat.Cat3)
	for _, CatBase := range cat.Cat3 {
		...
		//if len(CatBase.Cat3) != 0 {
		if CatBase.Cat3 != nil && len(CatBase.Cat3) != 0 {
			FindCat, ErrorFindCat := findCategory3Base_WithParam(CatBase, Parent, Name, Slug)
			//fmt.Println("---FindCat", FindCat, ErrorFindCat, " -- ", FindCat.Name, FindCat.Slug, FindCat.Parent)
			if ErrorFindCat == nil { // Если не нашёл
				return FindCat, nil
			}
