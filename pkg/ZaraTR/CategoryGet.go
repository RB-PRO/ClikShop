...
func LoadCategory() (cat Category, ErrorCat error) {
	...
	if res.StatusCode != http.StatusOK {
		return Category{}, fmt.Errorf("LoadCategory: wrong status code: %d", res.StatusCode)
	}
	...
	// Декодируем полученный json и получаем данные категорий
	err = json.Unmarshal(bodyBytes, &cat)
	if err != nil {
		return Category{}, err
	}
