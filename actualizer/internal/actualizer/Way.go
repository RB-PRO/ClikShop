...
func (s *Service) Sub(Folder string) error {
...
// Передаём путь к папке с файлами с товарами и эта функция обработает все товары из папки,
// удалив дубли и сохранит новые товары в новой папке `*_sub`
func (s *Service) DeleteRepeated(Folder string) error {
	folderSub := Folder + "_sub"
	folderDr := Folder + "_dr"
	...
	for _, file := range files {
		...
		for i := range Variety.Product {
			if _, ok := sku[Variety.Product[i].Article]; ok { // Если был уже такой SKU
				s.Gol.Warn(fmt.Sprintf("DeleteRepeated: %s: Товар %s с артикулом %s уже был, поэтому его игнорирую",
...
// Передаём путь к папке с файлами с товарами и эта функция обработает все товары из папки,
// удалив дубли и сохранит новые товары в новой папке `*_sub`
func (s *Service) Trans(Folder string) error {
	...
	for ifile, file := range files {
		...
		for i := range Variety.Product {
			...
			var ErrCoast error
			Variety.Product[i], ErrCoast = s.Coast(Variety.Product[i])
			if ErrCoast != nil {
				s.Gol.Err(fmt.Sprintf("[%d/%d]: %s: Товар '%s' %v",
					ifile, len(files), file, Variety.Product[i].Name, ErrCoast))
				continue
			}

			// Перевести товар
			TranslateProd, ErrorTranstate := s.Translate.TranslateProduct2(Variety.Product[i])
			if ErrorTranstate != nil {
				// fmt.Println("\nErrorTranstate ", ErrorTranstate, "\n ")
				s.Translate, _ = transrb.New(s.Translate.FolderID, s.Translate.OAuthToken)
				Variety.Product[i], _ = s.Translate.TranslateProduct2(Variety.Product[i])
			} else {
...
func (s *Service) Coast(prod bases.Product2) (bases.Product2, error) {
	var Walrus float64
	var Delivery int
	if cm, ok := s.mapCoast[prod.Manufacturer]; ok {
		Walrus = cm.Walrus
...
func (s *Service) subtraction(a bases.Variety2) (deductible bases.Variety2) {
	for i := range a.Product {
		if _, ok := s.SKU[a.Product[i].Article]; !ok {
...
func (s *Service) Push(Folder string) error {
	...
	for ifile := 0; ifile < len(files); ifile++ {
		...
		for i := range VarietyOrig.Product {
			if _, ok := sku[VarietyOrig.Product[i].Article]; !ok {
				ID, ErrAddProd := s.BitrixService.AddProduct(VarietyOrig.Product[i])
				if ErrAddProd != nil {
					s.Gol.Err(fmt.Sprintf("Ошибка при загрузке товара %s, Ошибка: %v",
						VarietyOrig.Product[i].Name, ErrAddProd))
					ErrProduct.Product = append(ErrProduct.Product, VarietyOrig.Product[i])
					ProductPushFalse++
				} else {
					s.Gol.Info(fmt.Sprintf("Загрузил товар %s, bx - https://213.226.124.16/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=15&type=aspro_lite_catalog&lang=ru&ID=%d&find_section_section=0&WF=Y , donor - %s",
				...
				s.Gol.Err(fmt.Sprintf("Товар '%s' с артикулом %s уже был при загрузке. Файл %s",
